...
projects_root_dir = Path("../../projects/")
...
def list_projects():
    ...
    projects = [p.name for p in projects_root_dir.iterdir() if p.is_dir()]
...
def init_project_endpoint():
    ...
    current_project = init_project(project_path)
...
def get_mappings():
    ...
    """
    return jsonify(get_mappings_int(current_project))
...
def get_mapping(id: str):
    ...
    mapping = get_mapping_int(current_project, id)
    if mapping:
        return mapping
    else:
        return "", 404
...
def get_mapping_fields(id: str):
    ...
    responses:
      ...
      404:
        description: Mapping not found
    """
    fields = get_mapping_fields_int(current_project, id)
    if fields:
        return fields
    else:
        return "", 404
...
@app.route("/mapping/<mapping_id>/field/<field_id>/classification", methods=["POST"])
def post_mapping_classification(mapping_id: str, field_id: str):
    ...
    try:
        result = post_mapping_classification_int(
            current_project, mapping_id, field_id, request.get_json()
        )
